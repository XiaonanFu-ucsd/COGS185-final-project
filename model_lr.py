...
class Attention(nn.Module):
    def __init__(self, config):
        ...
        self.wq0 = nn.Linear(config.n_embd, config.n_embd_lr, bias=False)
        self.wq1 = nn.Linear(config.n_embd_lr, config.n_embd, bias=False)
        self.wk0 = nn.Linear(config.n_embd, config.n_embd_lr, bias=False)
        self.wk1 = nn.Linear(config.n_embd_lr, config.n_embd, bias=False)
        self.wv0 = nn.Linear(config.n_embd, config.n_embd_lr, bias=False)
        self.wv1 = nn.Linear(config.n_embd_lr, config.n_embd, bias=False)
        # output projection
        self.wo0 = nn.Linear(config.n_embd, config.n_embd_lr, bias=False)
        self.wo1 = nn.Linear(config.n_embd_lr, config.n_embd, bias=False)
        # regularization
    ...
    def forward(self, x: Tensor, pos_emb: Tensor, start_idx: int):
        ...
        xq, xk, xv = self.wq1(self.wq0(x)), self.wk1(self.wk0(x)), self.wv1(self.wv0(x))
...
class MLP(nn.Module):
    ...
    def __init__(self, config):
        super().__init__()
        self.fc1 = nn.Linear(config.n_embd, 2 * config.n_embd_lr, bias=False)
        self.fc2  = nn.Linear(2 * config.n_embd_lr, int(config.n_embd*2.5), bias=False)
        self.norm = RMSNorm(int(config.n_embd*2.5))
        self.fc3 = nn.Linear(int(config.n_embd*2.5), 2 * config.n_embd_lr, bias=False)
        self.fc4  = nn.Linear(2 * config.n_embd_lr, config.n_embd, bias=False)
...
class GPT(nn.Module):
    def __init__(self, config: GPTConfig):
        ...
        self.emb_to_logits1 = nn.Linear(config.n_embd, config.n_embd_lr, bias=False)
        self.emb_to_logits2 = nn.Linear(config.n_embd_lr, config.vocab_size, bias=False)
    ...
    def forward(self, tokens, start_idx=0, training=True): # return all logits
        ...
        logits = self.emb_to_logits2(self.emb_to_logits1(self.transformer.ln_final(h)))
